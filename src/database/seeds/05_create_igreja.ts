...
export async function seed(knex: Knex) {
    await knex('cem_new.dadosIgreja as di')
        .join("cem_new.membros as m", "m.chEsEndereco", "di.id")
        .join("cem_testes.membros as mt", "mt.nome", "m.nome")
        .select("di.isBatizado as ehBatizado", "di.dataBatismo", "di.igrejaBatizado", "di.ultimoPastor", "di.ultimaIgreja", "mt.id as chEsMembro")
